...
#top_model_weights_path = 'bottleneck_fc_model_name.h5'


def predictName(path, top_model_weights_path, class_indices):
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    sess = tf.Session(config = config)
    
    # load the class_indices saved in the earlier step
    class_dictionary = np.load(class_indices).item()
